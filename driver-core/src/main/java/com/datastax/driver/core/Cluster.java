...
public class Cluster implements Closeable {
   ...
   */
  protected Cluster(
      String name, List<InetSocketAddress> contactPoints, Configuration configuration) {
  ...
  public static class Builder implements Initializer {
     ...
     * Adds contact points.
     *
     * <p>See {@link Builder#addContactPoint} for more details on contact points. Contrarily to
     ...
     *
     * <p>See {@link AddressTranslator} for more detail on address translation, but the default
     ...
     *
     * <p>This feature is only available with version {@link ProtocolVersion#V3 V3} or above of the
     ...
     *
     * <p>If no generator is set through this method, the driver will default to client-side
     ...
     *
     * <p>If no codec registry is set through this method, {@link CodecRegistry#DEFAULT_INSTANCE}
     * will be used instead.
     *
     * <p>Note that if two or more {@link Cluster} instances are configured to use the default codec
     ...
     *
     * <p>Use this method when a custom authentication scheme is in place. You shouldn't call both
     ...
     *
     * <p>Calling this method will use the JDK-based implementation with the default options (see
     * {@link RemoteEndpointAwareJdkSSLOptions.Builder}). This is thus a shortcut for {@code
     * withSSL(JdkSSLOptions.builder().build())}.
     *
     * <p>Note that if SSL is enabled, the driver will not connect to any Cassandra nodes that
     ...
     *
     * <p>Note: repeated calls to this method will override the previous ones.
     *
     ...
     *
     * <p>If no pooling options are set through this method, default pooling options will be used.
     *
     ...
     *
     * <p>If no socket options are set through this method, default socket options will be used.
     *
     ...
     *
     * <p>If no query options are set through this method, default query options will be used.
     *
     ...
     *
     * <p>If no options are set through this method, a new instance of {@link ThreadingOptions} will
     * be used.
     *
     ...
     *
     * <p>If no Netty options are set through this method, {@link NettyOptions#DEFAULT_INSTANCE}
   ...
   *
   * <p>Note: the reason we create a Manager object separate from Cluster is that Manager is not
  ...
  class Manager implements Connection.DefaultResponseHandler {
    ...
    final Set<SchemaChangeListener> schemaChangeListeners =
        new CopyOnWriteArraySet<SchemaChangeListener>();
    ...
    synchronized void init() {
      ...
      try {
        ...
        for (Host host : contactPointHosts) {
          if (!allHosts.contains(host)) removedContactPointHosts.add(host);
          else if (host.state == Host.State.DOWN) downContactPointHosts.add(host);
        }
    ...
    private Connection prepareAllQueries(Host host, Connection reusedConnection)
      ...
      try {
        ...
        for (String keyspace : perKeyspace.keySet()) {
          ...
          List<Connection.Future> futures =
              new ArrayList<Connection.Future>(preparedQueries.size());
    ...
    ListenableFuture<Void> submitSchemaRefresh(
  ...
  /**
   * Periodically ensures that closed connections are properly terminated once they have no more
   * pending requests.
   *
   * <p>This is normally done when the connection errors out, or when the last request is processed;
   * this class acts as a last-effort protection since unterminated connections can lead to
   * deadlocks. If it terminates a connection, this indicates a bug; warnings are logged so that
  ...
  static class ConnectionReaper {
    ...
    private final Runnable reaperTask =
        new Runnable() {
          @Override
          public void run() {
            long now = System.currentTimeMillis();
    ...
    ConnectionReaper(ScheduledExecutorService executor) {
      ...
      this.executor.scheduleWithFixedDelay(
          reaperTask, INTERVAL_MS, INTERVAL_MS, TimeUnit.MILLISECONDS);
    }
